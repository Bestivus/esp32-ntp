 ...
 * network, so a slow or unreachable broker cannot delay an NTP reply. The
 * wire-format codec (mqtt_wire.h/.c) is a small hand-rolled implementation
 * of exactly the packet types used here -- CONNECT/PUBLISH/PINGREQ/CONNACK,
 * QoS 0 only -- kept deliberately dependency-free rather than vendoring a
 * general-purpose MQTT client library for a handful of functions.
 */
class MqttPublisher {
public:
  MqttPublisher();

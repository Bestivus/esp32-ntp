...
**2026-08-13 update: rebased onto upstream's current `main`.** Upstream did a substantial rewrite
in the interim — replaced the vendored `ioLibrary_Driver` with a first-party W5500 driver
(`w5500_drv.c`/`.h`, `w5500_dhcp.c`/`.h`), split the stats server into `components/webui` with a
new NVS-backed settings/management UI, and independently landed several fixes that overlap with
